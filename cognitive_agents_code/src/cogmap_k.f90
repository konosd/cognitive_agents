...
program cognitive_maps
  ...
  integer, parameter    :: N_AGENT_STAT = 7 ! no of static / non-moving agents
  ...
  real              :: r_gyration_norm, stat_radius, log_fraction,XCOOR, mean_traj
  character(len=90) :: path, dir_name, file_name, arg_n_vtraj
  character(len=90) :: arg_n_agent, arg_n_vstep, arg_n_step, arg_stat_r, xcoor_agent
  ...
  integer :: path_sum, y_index, x_index, index, XCOOR_INT
  real, dimension(:), allocatable :: log_fractions, mean_log_fractions, v_coordinates2
  real, dimension(:,:), allocatable :: vr, forces
  ...
  call get_command_argument(5,xcoor_agent)
  ...
  coordinates(1,N_AGENT_STAT+1) = coordinates(1,N_AGENT_STAT+1) + XCOOR. 
  ...
  do step=1, N_STEP ! for every step in time
    ...
    ! do i=1, N_AGENT ! for every particle in that timestep
      ...
      if (CogmapTYPE=='R') then
        ...
        mean_traj = sum(mean_log_fractions)/N_VTRAJ
        ...
        do vtraj=1, N_VTRAJ
          ...
          gauss(1) = gauss(1) + v_noise(1,vtraj)*(mean_log_fractions(vtraj)/mean_traj)
          gauss(2) = gauss(2) + v_noise(2,vtraj)*(mean_log_fractions(vtraj)/mean_traj)
        end do
  ...
  print *, XCOOR ,sum(forces(:,1))/N_STEP, sum(forces(:,2))/N_STEP
