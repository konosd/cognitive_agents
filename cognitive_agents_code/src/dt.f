...
c  22 Dec 2018
       ...
       PROGRAM cognitive
        ...
        REAL, DIMENSION(:,:), ALLOCATABLE :: R,Rb,R2,V,Rvirt, vrecord
        ...
        CHARACTER(len=90)::np1,repultype,InfoFile,CoorFile, vrecordfile
      !  CHARACTER(len=90)::TempFile,CoorlFile
        CHARACTER(len=90)::tau_char,np_char,nsteps_char,file_nr
        CHARACTER(len=90)::frac_char, corfile
...
c       frac=0.1
        ...
        vrr=3  ! virtual traj record particle number (<np)
        ! Real trajectrories
...
c Setup
     ...
     *  trim(adjustl("_rt"))//trim(adjustl(repultype))
        ...
        write(corfile,'(a,a)')trim(path),"_cor.dat"
    ...
    !    OPEN(49,FILE=CoorlFile,form='formatted')
        ...
        OPEN(45,FILE=corfile,form='formatted')
...
c Main loop
        DO st=1,nsteps
            DO i=1,np
                ...
                DO tr=1,nvtraj
                    ...
                    Flx=-gamma*Vvirt_x+7*noi*Gausv_x(tr)
                    Fly=-gamma*Vvirt_y+7*noi*Gausv_y(tr)
        ...
        call repul_hcore(i,np,L,sigma,Ar,R2virt_x,R2virt_y,R2,
...
!     *  Frepx_virt,Frepy_virt)
                        ...
                        ! COR
                        if (((Frepx_p.eq.0).and.(Frepx_p.eq.0)).and.
     *((Frepx_virt.ne.0).or.(Frepy_virt.ne.0))) then
                            vb=sqrt(Vvirt_x**2+Vvirt_y**2)
                        end if

                        if (((Frepx_p.ne.0).or.(Frepy_p.ne.0)).and.
     *((Frepx_virt.eq.0).or.(Frepy_virt.eq.0))) then
                            va=sqrt(Vvirt_x**2+Vvirt_y**2)
...
c                           IF ((st.eq.1).and.(i.eq.vrr).and.(tr.eq.2))
c    *                      THEN
c                           print *,Rv_x, Rv_y ,  dtvirt
c    *                     ,floor(t_precise*frac),t_count
c                           END IF
                    ...
                    if (i.eq.vrr) then
                        do st_virt=1,tau
                        vrecord(tr,st_virt)=MODULO(Rvirt(st_virt,1),L)
                        vrecord(tr,tau+st_virt)=MODULO(Rvirt(st_virt,2
     *                  ),L)
                        end do
                    end if
       ...
       ! call repul_hcore(np,L,sigma,Ar,R2(i,1),R2(i,2),R2,Frepx,Frepy)
        ! Repulsion (soft)
        call repul_hcore(i,np,L,sigma,Ar,R2(i,1),R2(i,2),R2,Frepx,Frepy)
