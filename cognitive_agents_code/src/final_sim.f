...
c  22 Dec 2018
       ...
       PROGRAM cognitive
        ...
        INTEGER i,n,j,np,st,nsteps,tr,nvtraj,st_virt,tau,L2
...
c getting the command line arguments
        ...
        CALL GET_COMMAND_ARGUMENT(3,nsteps_char)   !load the command line arguments
...
c Analysis parameters
        ...
        start = 1        ! starting timestep for analysis ->> needs to be steady state
...
c Setup
     ...
     *  trim(adjustl("nsteps"))//trim(adjustl(nsteps1))
        ...
        DO n=1,10
            write(n_char,'(I5.5)')n ! formatting
...
c Analysis -> needs to set to steady state
     ...
     *    (mod(st,incr_coor).eq.0)) THEN
        ...
        IF(save_temp)skor=skor/(np)
        IF((save_temp).and.(mod(st,incr).eq.0))write(60,*)skor
