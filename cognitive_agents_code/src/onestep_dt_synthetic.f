...
c               ! Langevin equation
c               Flx=-gamma*V(i,1)+noi*Gaus_x
c               Fly=-gamma*V(i,2)+noi*Gaus_y


c       ! Repulsion (hard core)
c      ! call repul_hcore(np,L,sigma,Ar,R2(i,1),R2(i,2),R2,Frepx,Frepy)
c       ! Repulsion (soft)
c       call repul_soft(i,np,L,sigma,Ar,R2(i,1),R2(i,2),R2,Frepx,Frepy)
c       ! Repulsion (adjustable hard core)
...
c               ! Integration
c               Rtempx=2*R(i,1)-Rb(i,1)+Fx*dt**2
c               Rtempy=2*R(i,2)-Rb(i,2)+Fy*dt**2
c               V(i,1)=(Rtempx-R(i,1))/dt
c               V(i,2)=(Rtempy-R(i,2))/dt
c               Rb(i,1)=R(i,1)
c               Rb(i,2)=R(i,2)
c               R(i,1)=Rtempx
c               R(i,2)=Rtempy
c               R2(i,1)=MODULO(R(i,1),L)
c               R2(i,2)=MODULO(R(i,2),L)
            END DO
