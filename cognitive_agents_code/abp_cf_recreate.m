...
n_agent = [101 200 400 600 800 1000];       %number of agents
...
% -------------------------------------------------------------------------
function cog_force = calc_cogforce(agent, agent_coor, agent_velo, ...
    ...
    for traj = 1:n_trajectories
       [x,y,u,v,e,f_init,bound] = virtual_trajectory(agent,agent_coor, agent_velo,...
           agent_e, n_steps, diameter, area, strength, friction, D, h, repul_type, d2, a, c, q);
       mean_traj = [mean(x) mean(y)];
       gyration = ([x y] - mean_traj).^2;
       gyration =  (gyration(:,1)+gyration(:,2))/n_steps;
       gyration = sum(gyration);
       mean_traj = mean([(x-agent_coor(agent,1)) (y-agent_coor(agent,2))]);
       dotp = dot( f_init, mean_traj );
       gyration = log(gyration) * dotp/abs(dotp);
       cog_force = cog_force + 2* thermostat * gyration * f_init;
    end
...
function [x, y, u, v, e] = cog_force_em_solver(agent_coor, agent_velo,...
    agent_e,n_agent, n_steps, n_vsteps, diameter, area, strength, friction, D, h, repul_type, d2, a, c, q)
